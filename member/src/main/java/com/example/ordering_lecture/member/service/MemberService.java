...
import com.example.ordering_lecture.feign.NoticeServiceClient;
...
public class MemberService {
    ...
    public MemberService(MemberRepository memberRepository, PasswordEncoder passwordEncoder, SellerRepository sellerRepository, LikedSellerRepository likedSellerRepository, JwtTokenProvider jwtTokenProvider, NoticeServiceClient noticeServiceClient){
    ...
    public MemberLikeSellerResponseDto likeSeller(String email, Long sellerId) throws OrTopiaException {
        ...
        // 즐겨찾기 시 판매자 이메일 구독.
        log.info(seller.getMember().getEmail()+"의 구독 요청을 보냅니다.");
        log.info( noticeServiceClient.subscribeSellerEmail(seller.getMember().getEmail(),buyer.getEmail()));
    ...
    public void unlikeSeller(String buyerEmail, Long sellerId) throws OrTopiaException {
        ...
        // 즐겨 찾기 시 판매자 이메일 구독 취소.
        log.info(seller.getMember().getEmail()+"의 구독 취소 요청을 보냅니다.");
        log.info( noticeServiceClient.unsubscribeSellerEmail(seller.getMember().getEmail(),buyerEmail));
    }
    //판매자를 즐겨찾기한 구매자 목록
